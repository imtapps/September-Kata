...
class BowlingScoreTests(TestCase):
    ...
    def test_can_retrieve_saved_score(self):
        models.BowlingScore(name="eric", score="123").save()

        model = models.BowlingScore.objects.get(pk=1)
        self.assertEqual("eric", model.name)
        self.assertEqual("123", model.score)

    def test_can_retrieve_saved_scores_by_name(self):
        name = "eric"

        models.BowlingScore.objects.create(name=name, score="123")
        models.BowlingScore.objects.create(name=name, score="456")
        models.BowlingScore.objects.create(name="aaron", score="111")

        scores = models.BowlingScore.get_by_name(name)
